...
#include <stdlib.h>
#include <string.h>
\\to do:adicionar as outras bibliotecas necessárias 


int main() {
	

	int escolha; \\declara uma variavel para salvar a escolha do usuário
...
\\to do: implementar a função creat index
...
\\to do: implementar a função select
...
\\to do: implementar a função select where
		...
		case 4: \\se a funcionalidade escolhida for a numero 4:
			...
			SELECT_WHERE(arg1, arg2); \\chama a função de buscar e listar pessoas se baseando em critérios específicos passando os 3 argumentos necessários
			break;
